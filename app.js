...
const keyboard=$('keyboard'),keyboardWrap=$('keyboardWrap'),visual=$('visual'),listen=$('listen'),calibrate=$('calibrate'),status=$('status'),readout=$('readout'),latency=$('latency'),palette=$('palette'),colour=$('colour'),settingsButton=$('settingsButton'),settings=$('settings'),notice=$('startupNotice');
...
function focusKey(m){const k=keys.get(m);if(k)keyboardWrap.scrollLeft=Math.max(0,k.offsetLeft-keyboardWrap.clientWidth/2)}
function setDefaults(){if(!localStorage.getItem(SETTINGS)){confidence.value='58';frameConfidence.value='16'}}
function loadSettings(){try{const s=JSON.parse(localStorage.getItem(SETTINGS)||'null');if(!s)return;if(Number.isFinite(s.confidence))confidence.value=s.confidence;if(Number.isFinite(s.frameConfidence))frameConfidence.value=s.frameConfidence;if(Number.isFinite(s.keySize))keySize.value=s.keySize;if(/^#[0-9a-f]{6}$/i.test(s.glow||''))document.documentElement.style.setProperty('--glow',s.glow)}catch(e){}}
function saveSettings(){try{localStorage.setItem(SETTINGS,JSON.stringify({confidence:+confidence.value,frameConfidence:+frameConfidence.value,keySize:+keySize.value,glow:getComputedStyle(document.documentElement).getPropertyValue('--glow').trim()}))}catch(e){}}
...
function loadCalibration(){try{const x=JSON.parse(localStorage.getItem(CAL_KEY)||'{}');return x&&typeof x==='object'?x:{}}catch(e){return{}}}
...
function activate(m,c,sample){if(active.has(m))return;const t=sample/SR;if(t-(recent.get(m)||-Infinity)<.22)return;recent.set(m,t);const k=keys.get(m);if(k)k.classList.add('active');const b=makeBar(m,c),n=info(m);readout.textContent='Note: '+n.label;active.set(m,{m,key:k,bar:b,start:sample,release:0,sustained:false})}
...
async function stop(){listening=false;await closeAudio();listen.classList.remove('on');listen.textContent='Start listening';listen.disabled=false;status.textContent='Tap Start listening'}
async function startCalibration(){if(listening)await stop();calibrate.disabled=true;notice.style.display='none';status.textContent='Preparing calibration…';try{await openAudio();beginCalibration();calibrate.disabled=false}catch(e){finishCalibration();await closeAudio();calibrate.disabled=false;status.textContent='Calibration could not start: '+(e.message||e.name)}}
async function cancelCalibration(){finishCalibration();await closeAudio()}
listen.onclick=()=>listening?stop():start();calibrate.onclick=startCalibration;calCancel.onclick=cancelCalibration;calSkip.onclick=advanceCalibration;calReset.onclick=()=>{if(confirm('Delete all saved piano calibration?')){calibration={};saveCalibration();calProgress.textContent='0 of 88 keys saved'}};colour.onclick=()=>{palette.classList.toggle('open');settings.classList.remove('open')};settingsButton.onclick=()=>{settings.classList.toggle('open');palette.classList.remove('open')};[confidence,frameConfidence,keySize].forEach(e=>{e.oninput=updateSettings;e.onchange=updateSettings});keyboardWrap.addEventListener('scroll',syncBars,{passive:true});window.addEventListener('resize',syncBars);window.addEventListener('pagehide',()=>{saveSettings();closeAudio()});buildKeyboard();buildPalette();setDefaults();loadSettings();updateSettings();setTimeout(()=>{focusKey(60);syncBars()},100);
}());