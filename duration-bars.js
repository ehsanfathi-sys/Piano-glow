...
function frame(now){const maxHeight=Math.max(MIN_HEIGHT,visual.clientHeight*.72);for(const[bar,state]of tracked){if(!bar.isConnected){tracked.delete(bar);continue}if(!state.released){if(keyIsActive(state.midi)){const seconds=Math.max(0,(now-state.start)/1000);const height=Math.min(maxHeight,MIN_HEIGHT+seconds*GROWTH_PER_SECOND);bar.style.height=height+'px';bar.style.minHeight=height+'px'}else releaseBar(bar,state)}}requestAnimationFrame(frame)}
